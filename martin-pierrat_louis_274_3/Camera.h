...
public:
    ...
    enum
    {
        ...
        RENDER_BIDIRECTIONAL_PATHTRACER = 4,
        RENDER_PHOTON_MAPPING = 5
    };
    ...
    void    environment_map(Vector3 *img, const MappingType &map_type, int w, int h)
    {
    ...
    Vector3 bgColor(const Vector3 &ray) const;

    std::pair<int, int>     world_point_to_camera_point(const Vector3 &point);

private:

    int m_w;
    int m_h;
    MappingType m_map_type;
    Vector3 *m_img;
