...
public:
    ...
    */
    ...
    static Vector3 refract(const Vector3 &N, const Vector3 &I, float n);

    static Vector3 reflect(const Vector3 &I, const Vector3 &N);

    static Vector3 spherical_random(void);
    static Vector3 hemispherical_random(void);
    //! Length<sup>2</sup>.
    /*!
...
Vector3::refract(const Vector3 &N, const Vector3 &I, float n)
{
    ...
    const float k = 1.0f - n * n * (1.0f - dNI * dNI);
    if (k < 0.0f) {
        return Vector3(0.0f);
    }
    return n * I - (n * dNI + sqrt(k)) * N;
}
...
inline Vector3
Vector3::spherical_random(void)
{
    const auto random = [](float min, float max) -> float {
        float p = (double)(rand()) / RAND_MAX;
        if (max < min) std::swap(max, min);
        return p * (max - min) + min;
    };
    const float z = random(-1.0f, 1.0f);
    const float a = random(0.0f, 6.283185307179586476925286766559f); // phi
...
inline Vector3
Vector3::hemispherical_random(void)
{
    const Vector3   &r = Vector3::spherical_random();
    return Vector3(r.x, r.y, fabs(r.z));
}
